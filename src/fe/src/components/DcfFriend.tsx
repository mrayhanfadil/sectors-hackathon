...
export function DcfFriend({ ticker }: DcfFriendProps) {
  ...
  const ratingColorClass = (() => {
    ...
    if (r === "BUY") return "bg-emerald-50 border-emerald-300 text-emerald-950"
    if (r === "HOLD") return "bg-amber-50 border-amber-300 text-amber-950"
    if (r === "SELL") return "bg-red-50 border-red-300 text-red-950"
  ...
  return (
    <details className="group rounded-xl border border-slate-200 bg-white shadow-sm">
      <summary className="flex cursor-pointer items-center justify-between p-4 font-semibold text-slate-800 hover:bg-slate-50 select-none">
      ...
      <div className="border-t border-slate-100 p-4 space-y-6">
        ...
        {error && (
          <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800 space-y-3">
            ...
            <Button size="sm" variant="outline" onClick={loadData} className="bg-white hover:bg-red-50">
              Coba Lagi
            </Button>
          </div>
        )}
          ...
          <>
            ...
            {rec && (
              <div className={`rounded-xl border p-4 ${ratingColorClass}`}>
                ...
                {rec.reason_override && (
                  <div className="mt-3 rounded-lg border-l-4 border-slate-700 bg-white/70 p-3 text-xs text-slate-800">
                    <div className="font-semibold text-slate-900 mb-1">Catatan Gate Review Required:</div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                        ...
                        {data.wacc_table.map((row, idx) => {
                          ...
                          return (
                            <tr
                              ...
                              className={`border-b ${
                                isLast
                                  ? "bg-sky-100 font-bold text-slate-900"
                                  : idx % 2 === 1
                                  ? "bg-slate-50"
                                  : "bg-white"
                              }`}
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Skenario Bear / Base / Bull</h3>
                      ...
                      <div key={scName} className={`rounded-xl p-3.5 ${cardBg}`}>
                        ...
                        <div
                          className={`mt-2 grid grid-cols-3 gap-2 border-t pt-2 text-[11px] ${
                            isBase ? "border-slate-700 text-slate-300" : "border-slate-200 text-slate-600"
                          }`}
                        >
                          <div>
            ...
            {data.sensitivity && data.sensitivity.fair_value && (
              <Card>
                ...
                <CardContent className="p-0">
                        ...
                        {data.sensitivity.wacc_axis.map((w, rowIdx) => {
                          ...
                          return (
                            <tr key={w} className="border-b">
                              <td className="py-2 px-3 text-left font-bold font-mono bg-slate-50 text-slate-900">
                                {(w * 100).toFixed(2)}%
                              </td>
                              {rowVals.map((val, colIdx) => {
                                ...
                                return (
                                  <td
                                    key={colIdx}
                                    className="py-2 px-3 font-mono border-r border-slate-100 last:border-r-0"
                  ...
                  <div className="flex flex-wrap items-center justify-between p-3 text-[11px] text-slate-500 border-t bg-slate-50">
                    <div className="flex items-center gap-2">
