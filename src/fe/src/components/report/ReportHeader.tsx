...
export function ReportHeader({
  ...
  const tabActive = "border-[#0a0a0a] text-[#0a0a0a]"
  const tabIdle = "border-transparent text-neutral-500 hover:text-[#0a0a0a]"

  return (
    <div className="sticky top-[57px] z-20 -mx-4 -mt-6 mb-6 border-b border-neutral-200 bg-white/95 backdrop-blur">
      <div className="mx-auto max-w-6xl px-4 pt-3">
        {/* Breadcrumb */}
        <Link
          to="/"
          className="inline-flex items-center gap-1 text-xs font-medium text-[#0070f3] hover:underline"
        >
        ...
        <div className="mt-1.5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-neutral-200 bg-neutral-50 text-sm font-bold tracking-tight text-[#0a0a0a]">
              {logoInitial}
            </div>
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                {price != null && (
                  <span className="font-mono text-xl font-semibold tabular-nums tracking-tight text-[#0a0a0a]">
                    Rp {fmtIDR(price)}
                  </span>
                )}
                {upside && (
                ...
                {template && (
                  <span className="rounded border border-neutral-200 bg-white px-1.5 py-px font-mono text-[10px] uppercase tracking-wider text-neutral-500">
                    {template}
                  </span>
                )}
              </div>
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            <Button
              ...
              disabled={isPdfBusy}
              className="h-8 gap-1.5 border-neutral-200 text-xs text-neutral-700 hover:text-[#0a0a0a] cursor-pointer"
            >
              {isPdfBusy ? (
            ...
            <Link
              ...
              className="inline-flex h-8 items-center gap-1.5 rounded-md border border-neutral-200 bg-white px-3 text-xs font-medium text-[#0070f3] hover:bg-neutral-50 transition-colors"
            >
