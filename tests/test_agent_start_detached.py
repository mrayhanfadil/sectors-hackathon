...
def test_start_returns_quickly():
    ...
    t0 = time.time()
    r = requests.post(f"{API}/api/agent/start", json={"ticker": "BBCA"}, timeout=5)
    elapsed = time.time() - t0
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["ok"] is True
    assert body["status"] == "started"
    assert body["run_id"].startswith("bbca-")
...
def test_status_endpoint_exists():
    ...
    start_r = requests.post(f"{API}/api/agent/start", json={"ticker": "BBCA"}, timeout=5)
