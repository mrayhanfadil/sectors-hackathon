...
def build_statements_page(ticker: str = "AMMN", spine: Optional[dict] = None,
    ...
    def rows3(specs: list) -> list:
        ...
        for label, key, kind, note in specs:
            # "key" is the stable machine identifier - downstream exhibits
            # (slide7, house-rules Exhibit 17 checks) must look rows up by key,
            # never by label, so retitles don't break cross-sheet math.
            out.append({"label": label, "key": key, "cells": series(key), "kind": kind, "note": note})
        return out

    income = rows3([
        ("Revenue / Sales", "rev", "", ""),
        ("Beban pokok pendapatan", "cogs", DED, ""),
        ("Laba kotor", "gp", SUB, ""),
        ("Beban usaha & operasional", "opex", DED, ""),
        ("Laba usaha (EBIT)", "ebit", SUB, ""),
        ("Pendapatan bunga", "ii", NA, ""),
        ("Beban bunga", "ie", DED, ""),
        ("Pendapatan/(beban) non-operasional lainnya", "other", "", ""),
        ("Laba sebelum pajak", "ebt", SUB, ""),
        ("Beban pajak penghasilan", "tax", DED, ""),
        ("Kepentingan non-pengendali", "mino", "", ""),
        ("Laba bersih periode berjalan", "net", HL, ""),
    ])
    balance = rows3([
        ("ASSETS", "", "section", ""),
        ("Cash & Cash Equivalents", "cash", "", ""),
        ("Trade Receivables", "ar", NA, ""),
        ("Inventory", "inv", "", ""),
        ("Other Current Assets", "other_ca", "", ""),
        ("Total Current Assets", "tca", SUB, ""),
        ("Fixed Assets (Net)", "fixed", "", ""),
        ("Other Non-Current Assets", "other_nca", "", ""),
        ("Total Assets", "ta", SUB, ""),
        ("LIABILITIES & EQUITY", "", "section", ""),
        ("Short-term Debt", "st", "", ""),
        ("Trade Payables", "ap", NA, ""),
        ("Other Current Liabilities", "other_cl", "", ""),
        ("Total Current Liabilities", "tcl", SUB, ""),
        ("Long-term Debt", "lt", "", ""),
        ("Other Non-Current Liabilities", "other_ncl", "", ""),
        ("Total Liabilities", "tl", SUB, ""),
        ("Shareholders' Equity", "eq", "", ""),
        ("Total Liabilities & Equity", "tle", SUB, ""),
    ])
    ...
    return {
        ...
        "balance": {"exhibit_key": "balance", "title": f"Neraca keuangan ({YEARS[0]}–{YEARS[-1]})",
